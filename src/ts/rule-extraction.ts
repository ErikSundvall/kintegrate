import type { CategoryKey, RuleEntry } from './types';
...
function getCategory(value: unknown): CategoryKey | undefined {
...
export function oppositeValue(value: unknown): unknown {
...
export function extractConditionValue(statement: unknown): unknown {
...
export function deriveVisibilityValues(
  actionName: string,
  triggerValue: unknown
): { showValue: unknown; hideValue: unknown } | null {
...
export function extractRulesFromConditions(
...
export function pushValidationRangeRule(
  rules: RuleEntry[],
...
export function extractQuantityUnitRules(inputs: unknown[]): Array<{ unit: string; range: unknown }> {
  const unitInput = (inputs || []).find((input) => isRecord(input) && input.suffix === 'unit');
  if (!isRecord(unitInput) || !Array.isArray(unitInput.list)) {
    return [];
  }

  const output: Array<{ unit: string; range: unknown }> = [];
