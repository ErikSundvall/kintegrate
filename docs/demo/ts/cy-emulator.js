...
exports.FORM_READY_TIMEOUT_MS = void 0;
exports.getFormTestApi = getFormTestApi;
exports.setFormTestApiResolverForTests = setFormTestApiResolverForTests;
exports.createCyEmulator = createCyEmulator;
const POLL_INTERVAL_MS = 100;
exports.FORM_READY_TIMEOUT_MS = 10000;
...
function createCyEmulator() {
    ...
    const emulator = {
        fillField(path, value, opts = {}) {
            return enqueue(() => {
                getFormTestApi().setFieldValue(path, value, opts.multiIndex, opts.searchWithinContainerTag, opts.containerMultiIndex);
            });
        },
        ...
        expectValue(path, expected, opts = {}) {
            return enqueue(() => {
                const actual = getFormTestApi().getFieldValue(path, opts.multiIndex, opts.searchWithinContainerTag, opts.containerMultiIndex, opts.simpleValue);
                chai.expect(actual, `${path} value`).to.deep.equal(expected);
            });
        },
        assertRangeSamples(opts = {}) {
            const { path, label = 'range rule', min = null, max = null, minOp = '>=', maxOp = '<=', validSamples = [], invalidSamples = [], expectedUnit = null } = opts;
            return enqueue(() => {
                chai.expect(validSamples.length, `${label} generated valid samples`).to.be.greaterThan(0);
                validSamples.forEach((sample) => {
                    let actual = sample;
                    if (path) {
                        getFormTestApi().setFieldValue(path, sample, opts.multiIndex, opts.searchWithinContainerTag, opts.containerMultiIndex);
                        actual = getFormTestApi().getFieldValue(path, opts.multiIndex, opts.searchWithinContainerTag, opts.containerMultiIndex, opts.simpleValue);
                    }
                    chai.expect(isValidSample(actual, min, max, minOp, maxOp, expectedUnit), `${label} expected valid sample to pass`).to.equal(true);
                    if (expectedUnit) {
                ...
                invalidSamples.forEach((sample) => {
                    ...
                    chai.expect(isValidSample(actual, min, max, minOp, maxOp, expectedUnit), `${label} expected invalid sample to fail`).to.equal(false);
                });
            });
        },
        resetForm() {
        ...
        formViewerReady(timeoutMs = exports.FORM_READY_TIMEOUT_MS) {
            return enqueue(async () => {
                await waitFor(() => getFormTestApi().isReady() === true, timeoutMs, `Form viewer not ready after ${timeoutMs}ms`);
            });
        },
        waitForFormTestApi(timeoutMs = exports.FORM_READY_TIMEOUT_MS) {
            return enqueue(async () => {
                await waitFor(() => Boolean(resolveFormTestApiHost()), timeoutMs, `Form viewer not open or formTestApi not available after ${timeoutMs}ms`);
            });
        },
        getSkipReason() {
            return skipReason;
...
if (browserWindow) {
    browserWindow.CyEmulatorModule = {
        createCyEmulator,
        getFormTestApi,
        FORM_READY_TIMEOUT_MS: exports.FORM_READY_TIMEOUT_MS
    };
}
